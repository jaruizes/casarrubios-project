...
import com.jaruiz.casarrubios.recruiters.services.newpospublisher.utils.kafka.PositionDLQConsumer;
...
class NewPositionPublisherTest {
    ...
    @Inject PositionDLQConsumer positionDLQConsumer;
...
//        List<String> topicsCreated = KafkaAdminHelper.getTopics(KafkaContainer.getKafkaContainer().getBootstrapServers());
//        topicNames.forEach(topic -> {
//            assertTrue(topicsCreated.contains(topic));
//        });
    }
    ...
    public void tearDown(){
        String bootstrapServers = KafkaContainer.getKafkaContainer().getBootstrapServers();
        KafkaAdminHelper.cleanTopics(bootstrapServers, getTopicNames());
    }
    ...
    void whenARightPositionAndReqsAndTasksAndBenefitsAreReplicated_thenACompletePositionIsPublished() throws InterruptedException {
        ...
        final Position positionDQL = positionDLQConsumer.getPositionDLQ();

        assertNull(positionDQL);
    ...
    private List<String> getTopicNames() {
        return List.of(
            ...
            topologyConfig.getNewPositionsPublishedDLQTopic());
    }
}
