...
export class ApplicationsService {
  ...
  async getAllApplicationsByPositionId(
    ...
    this.logger.debug(
      `[Applications Service] Trying to fetch applications from: ${url}`,
    );
  ...
  async getApplicationCV(
    ...
    try {
      ...
      this.logger.debug(
        `[Applications Service] Received CV file with content type: ${response.headers['content-type']}`,
      );
      ...
      if (error.isAxiosError && error.response?.status === 404) {
        throw new NotFoundException(
          `CV for application with id ${applicationId} not found`,
        );
      }
